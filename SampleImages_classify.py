...
def day_night_image_classify_url(txt_url, repo_url, model, feat_scaler):
    ...
    for img_byte in img_all_bytes:
        ...
        if yhat == 1:
            ...
            urllib.urlretrieve(os.path.join(repo_url, img_file), './classified_images/day/' + img_file)
        else:
            night_images.append(img_file)
            urllib.urlretrieve(os.path.join(repo_url, img_file), './classified_images/night/' + img_file)
...
def load_models():
    ...
    model = pickle.load(open('saved_svm_model', 'rb'))
...
def main():
    ...
    repo_url = 'https://storage.googleapis.com/arcules-sample-images/'
    txt_url = 'https://storage.googleapis.com/arcules-sample-docs/list_file.txt'

    shutil.rmtree('./classified_images')
    ...
    for day_image in day_images:
        print('https://storage.googleapis.com/arcules-sample-images/' + day_image)
    print('--- List of URLs of night-time images ---')
    for night_image in night_images:
        print('https://storage.googleapis.com/arcules-sample-images/' + night_image)
